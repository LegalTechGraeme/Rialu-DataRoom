      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="page-subtitle">
            Your {BRAND_NAME} diligence workspace — open a matter, upload documents, and run AI
            review.
          </p>
        </div>
      ...
      <div className="mt-8 space-y-3">
        ...
        {matters?.map((m) => (
          <Link
            key={m.id}
            to={`/matters/${m.id}`}
            className="card block p-5 transition hover:border-brand/35 hover:shadow-md"
          >
            <div className="flex items-start justify-between gap-4">
              ...
              {m.clientRef ? (
                <span className="pill shrink-0">{m.clientRef}</span>
              ) : null}
            </div>
